...
def _find_column(columns: pd.Index, candidates: list[str]) -> str | None:
    """
    Find the first matching column in an Alpha Vantage response.

    Alpha Vantage digital currency endpoints can expose slightly different
    labels depending on the payload shape, for example:
      - '1. open'
      - '1a. open (USD)'
      - '4. close'
      - '4a. close (USD)'
    ...
    for candidate in candidates:
        exact = normalized.get(candidate.lower().strip())
    ...
    for col in columns:
        ...
        for candidate in candidates:
            cand_norm = candidate.lower().replace(" ", "")
...
def fetch_alpha_vantage_data(symbol: str, market: str, api_key: str) -> pd.DataFrame:
    ...
    for canonical, candidates in column_candidates.items():
        source_col = _find_column(df.columns, candidates)
        if source_col is None:
            missing_sources.append(canonical)
        else:
            rename_map[source_col] = canonical

    if missing_sources:
        raise ValueError(
            f"Missing expected columns from API response: {missing_sources}. "
    ...
    # Alpha Vantage returns numbers as strings; coerce safely and drop invalid rows.
    for col in df.columns:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df.index = pd.to_datetime(df.index, errors="coerce")
    df = df.loc[df.index.notna()].sort_index().dropna()

    if df.empty:
        raise ValueError("No valid rows remained after parsing the Alpha Vantage response.")
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    band_width = (features["Upper_Band"] - features["Lower_Band"]).replace(0, np.nan)
    features["Bollinger_PercB"] = (features["close"] - features["Lower_Band"]) / band_width
    ...
    if len(features) < 100:
        raise ValueError(
            f"Not enough rows after feature engineering ({len(features)}). "
            "Try a longer history or a smaller set of indicators."
        )
